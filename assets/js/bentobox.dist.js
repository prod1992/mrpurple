...
/***/ function(module, exports, __webpack_require__) {
	...
	var Forms = (function() {
		...
		forms.hideMessages = function() {
			$(forms.options.success).hide();
			$(forms.options.error).hide();
		};

		forms.formSubmitSuccess = function(data) {
			if (data.success == true || data.success == "true") {
				$(forms.options.success).fadeIn();
				forms.options.successCallback();
				forms.currentForm.trigger('reset');
			} else {
				forms.currentForm.find(forms.options.error).fadeIn();
				forms.options.errorCallback();
			}
		};

		forms.formSubmitError = function() {
			$(forms.options.error).fadeIn();
		};

		forms.initialize = function(userOptions) {
			for (var attributeName in userOptions) {
				forms.options[attributeName] = userOptions[attributeName];
			}
			console.log(forms.options);
			$(forms.options.formSelector).on('submit', forms.handleSubmit);
		};
